...
Each chapter tackles different questions related to the multimodal mobility options. First, we cover the existing literature in the domain of multimodal urban network, spanning from the infrastructure and dynamics to the available measures, tools and data to work with such phenomena. Second, we propose a new method to extract the multimodal profile from a city's multiplex transport network, and show how it can be applied to identify similarities between cities. Third, we zoom-in into the bicycle layer of the multimodal network, to show how fragmented it is, and propose an algorithmic approach to find the missing links. After applying the algorithms to different cities we show that a focalised investment, can have a significant impact improving the bicycle infrastructure connectivity. Moreover, we present a data-driven, network-based method to quantify the liveability of a city based on pedestrian accessibility to amenities and services, safety and environmental variables. Finally, we discuss the main contributions of this thesis and outline possible applications and open questions for the future.
